...
def main():
	...
	for idx, col in enumerate(Cols[:-1]):
	  for d in range(0,n_cases-1):
            check_num = is_number(col[d])  
            if(check_num == True):
              is_Int = isinstance(col[d], int)
              is_Float = isinstance(col[d], float)

              if(is_Int == True):  
	        interval = int(col[d+1]) - int(col[d])	  
	        if(interval != 0):
	          cutpoint = int((col[d+1]) + int(col[d]))/2	
	          ...
	          if(interval != 0):
	            ...
	            if(cutpoint not in cutpoints[idx]):
                        cutpoints[idx].append(cutpoint)
	                cutpoints.append([])
	...
	cutpoints = [x for x in cutpoints if x != []]
        ...
        for idx, cp in enumerate(cutpoints):
            bins =  sorted(cutpoints[idx], key=float)
            print "Got the bins"
            print bins
            d_Cols[idx] = numpy.digitize(Cols[idx], bins)
            rd_Cols[idx] = pd.cut(Cols[idx], len(bins))
            ...
            if(cs[i] == True):
                ...
                for v in conceptual_vars[j][0]:
                    ...
                    for x in range(0, len(sub[i][v-1])):
                        ...
                        rule.append(r)
                        if(x != len(sub[i][v-1])-1):
                            rule.append('&')
                    print tuple(rule),'->','( Decision,',list_of_concepts[j],')'

                    #fo = open("Rules.txt", "a")
                    #fo.write("Writing the rules\n");
                    #fo.close()
            else:
                next_glob = list(glob)
                pop_n = pop_n + 1
